...
// Tiles: critical cases, scheduled conferences, awaiting documents, resolved.
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setDocs(docRes.data);
        setCriticalCases(casesRes.data);
      } catch (err) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="divide-y divide-slate-100">
            ...
            {!loading && criticalCases.length === 0 && <p className="p-6 text-center text-slate-400 text-sm">No critical cases — great work.</p>}
            {criticalCases.slice(0, 6).map((c) => (
