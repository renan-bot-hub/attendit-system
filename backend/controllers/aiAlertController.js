...
const classificationModel = require('../ml/classificationModel');
...
const {
  ...
  normalizeRiskLevel,
  riskQueryValues,
} = require('../utils/riskLevels');

classificationModel.load().catch((err) => {
  console.warn('[ai-alerts] TensorFlow classification model unavailable - using rule-engine fallback.', err.message);
});
...
async function buildAlert(student, signals, s) {
  ...
  try {
    const pred = await classificationModel.predict({
    ...
    score = pred.score;
    tier  = normalizeRiskLevel(pred.tier);
    modelProbabilities = normalizeProbabilityMap(pred.probabilities);
    modelVersion = pred.modelVersion || classificationModel.meta()?.version || '';
  } catch (err) {
    const r = ruleScore(signals);
    score = r.score;
    tier  = normalizeRiskLevel(r.tier);
    scorer = 'rules';
  }
...
exports.runAnalysis = async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      created, refreshed,
      scorer: { model: modelHits, rules: ruleHits, modelReady: classificationModel.isReady() },
    });
  } catch (err) {
    console.error(err);
...
exports.listAlerts = async (req, res) => {
  try {
    ...
    const alerts = await AIAlert.find(filter)
      .populate('student', 'name email studentId section gradeSection gradeLevel parentName parentEmail parentPhone')
      .populate('linkedCase', 'status riskLevel')
      .sort({ flaggedOn: -1 })
      .lean();
    res.json(alerts.map((alert) => ({
      ...alert,
      riskLevel: normalizeRiskLevel(alert.riskLevel),
      modelProbabilities: normalizeProbabilityMap(alert.modelProbabilities),
      linkedCase: alert.linkedCase
        ? { ...alert.linkedCase, riskLevel: normalizeRiskLevel(alert.linkedCase.riskLevel) }
        : alert.linkedCase,
    })));
  } catch (err) {
...
exports.updateAlert = async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      modelProbabilities: normalizeProbabilityMap(data.modelProbabilities),
    });
  } catch (err) {
    res.status(500).json({ message: 'Server error', error: err.message });
  }
...
exports.escalateAlert = async (req, res) => {
  ...
  try {
    ...
    alert.riskLevel = normalizeRiskLevel(alert.riskLevel);
    ...
    const newCase = await Case.create({
      ...
      status: alert.riskLevel === 'High' ? 'Escalated' : 'Open',
      consecutiveAbsences: 0,
      openedBy: req.user.id,
      escalatedTo: null,
      escalatedAt: alert.riskLevel === 'High' ? new Date() : null,
