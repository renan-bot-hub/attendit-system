...
function analyseStudent(records, totalSessions = 0) {
  ...
  const last7DayAbsences = sorted.filter(
    (r) =>
      r.status === 'Absent' &&
      (now - new Date(r.timestamp).getTime()) <= 7 * day
  ).length;

  const last30DayAbsences = sorted.filter(
    (r) =>
      r.status === 'Absent' &&
      (now - new Date(r.timestamp).getTime()) <= 30 * day
  ).length;

  return {
    consecutive,
    worstDow,
    worstDowRate: worstRate,
    total,
    absent,
    late,
    attendanceRate,
...
function detectPattern(signals, s) {
  ...
  if (signals.absent >= s.warningTotalAbsences) {
    ...
    return {
      pattern: 'Total Absences Trend',
      patternDetail: `${signals.absent} total absences — approaching high risk`,
      recs,
    };
  }
...
async function buildAlert(student, signals, s) {
  ...
  try {
    ...
    score = pred.score;
    tier  = pred.tier === 'Critical' ? 'High Risk' : pred.tier;
...
exports.escalateAlert = async (req, res) => {
  ...
  try {
    ...
    if (alert.linkedCase) {
      return res.status(400).json({ message: 'Alert already escalated' });
    }
    ...
    const newCase = await Case.create({
      ...
      riskLevel: alert.riskLevel === 'Critical' ? 'High Risk' : alert.riskLevel,
      status: alert.riskLevel === 'High Risk' || alert.riskLevel === 'Critical' ? 'Escalated' : 'Open',
      consecutiveAbsences: 0,
      openedBy: req.user.id,
      escalatedTo: null,
      escalatedAt: alert.riskLevel === 'High Risk' || alert.riskLevel === 'Critical' ? new Date() : null,
      sourceAlert: alert._id,
    });

    alert.status = 'Actioned';
    alert.riskLevel = alert.riskLevel === 'Critical' ? 'High Risk' : alert.riskLevel;
